...
import exampleEventLog from '../data/renamed_log.csv';
import exampleVertices from '../data/renamed_log_vertices.csv';
...
function VisualizationPage() {
  ...
  // Загрузка примера данных при монтировании компонента
  useEffect(() => {
    const loadExampleData = async () => {
      // Проверяем, есть ли уже данные в localStorage или загружены ли примеры
      const storedLogs = localStorage.getItem('eventlogs');
      const storedVertices = localStorage.getItem('vertices');
      var newEventLog;
      if (!storedLogs || JSON.parse(storedLogs).length === 0) {
        // Загружаем пример event log
        const response = await fetch(exampleEventLog);
        ...
        // Загружаем пример vertices
        const verticesResponse = await fetch(exampleVertices);
        const verticesText = await verticesResponse.text();
        Papa.parse(verticesText, {
          header: false,
          dynamicTyping: true,
          complete: (results) => {
            // Обработка vertices данных (аналогично parseVertices функции)
            const vertices_ = [];
            const columns = results.data.map(item => item);
            for (let i = 0; i < Object.keys(columns[0]).length - 1; i++) {
              vertices_.push({
                'source': 'col ' + columns[0][i],
                'target': 'col ' + columns[0][i+1],
                'value': localStorage.getItem('maxEl'),
                'valueLabel': localStorage.getItem('maxEl')
              });
            }
            for (let i = 1; i < Object.keys(columns).length; i++) {
              for (let j = 0; j < Object.keys(columns[i]).length - 1; j++) {
                vertices_.push({
                  'source': `col${j+1} ` + columns[i][j],
                  'target': `col${j+2} ` + columns[i][j+1],
                  'value': localStorage.getItem('maxEl') / 1.5,
                  'valueLabel': localStorage.getItem('maxEl') / 1.5
                });
              }
            }
            ...
            const newVertices = {
              id: newEventLog.id + 'v',
              name: 'Example Vertices',
              data: vertices_,
              resultData: results
            };
            setVerticesOrder([newVertices]);
  ...
  return (
    <div>
        ...
        <nav className={sidebar ? 'nav-menu active' : 'nav-menu'}>
          <ul className='nav-menu-items' >
            ...
            <li className='nav-text' color="white">
              <span> The degree of accuracy of the visualization of transitions </span>
            </li>
        ...
        <li className='nav-text' color="white">
              <span> The degree of accuracy of event visualization </span>
        </li>
        <Grid container spacing={2} alignItems="center" justify="center">
          <Grid item xs={9}>
            <SankeySlider
        ...
        <nav className={leftSidebar ? 'nav-menu-left active' : 'nav-menu-left'}>
          <ul className='nav-menu-items' >
            ...
            */}
            ...
            <Box m={2} pt={2}>
            <Typography variant="h8" style={{ color: '#fff' }}>Event logs</Typography>
            ...
            return (
              ...
              <div key={eventlog.id}>
                ...
                <Typography variant="caption" style={{ color: '#fff', textDecoration: 'underline', cursor: 'pointer' }} onClick={() =>
                    ...
                    {verticesOrder.find(x => x.id === eventlog.id + 'v') ? verticesOrder.find(x => x.id === eventlog.id + 'v').name : "Add the vertex order"}
        ...
        <Box m={2} pt={2}>
          <Button variant="text" onClick={() => { document.getElementById('eventlogInput').click();}}>Add an event log
        ...
        <FormGroup>
          ...
          sx={{
            ...
            />} label= {<Typography variant="h8" color={'white'}>First subset</Typography>} 
          />
          ...
          sx={{
            ...
            />} label= {<Typography variant="h8" color={'white'}>Second subset</Typography>} 
            />
